...
function readFileContent(file) {
  try {
    ...
    // For xlsx, pdf, etc - send as base64
    return {
      type: 'base64',
      ext: ext,
...
// Build the Anthropic messages array with files
...
Return ONLY the complete HTML. Nothing before or after it.`
  ...
  for (const fc of fileContents) {
    ...
    if (fc.type === 'text') {
      ...
      const mediaType = fc.ext === '.pdf' ? 'application/pdf'
        : fc.ext === '.xlsx' ? 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
      ...
      if (fc.ext === '.pdf') {
        ...
        // For xlsx/csv send as text description
        content.push({
          type: 'text',
          text: `\n--- ATTACHED FILE: ${fc.name} (${fc.ext}) — base64 encoded, ${Math.round(fc.content.length * 0.75 / 1024)}KB ---\n[File attached as ${fc.ext} — extract all financial data from this file]\n`
        })
      }
    }
  }
...
async function saveToSupabase(fields, dashboardHtml) {
  try {
    ...
    if (clientId) {
      ...
      // Save submission with generated dashboard
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // If still no valid HTML, return error
    if (!html.includes('<!DOCTYPE')) {
      return res.status(500).json({ error: 'Claude did not return valid HTML. Please try again.' })
    }

...
IMPORTANT: Return ONLY raw HTML starting with <!DOCTYPE html>. No markdown. No backticks. No explanation. Just the HTML.
...
9 TABS:
1. Financial Performance - KPIs: Gross Revenue, COGS, Rent (benchmark 5-8%), People Cost, EBITDA. Monthly P&L table. Revenue trend chart. Service category donut. NO waterfall.
2. Revenue by Provider - unique color per provider, Avg Client Transaction, Hospital Discounts KPI
3. Staffing & Payroll - comp breakdown, OT KPI box, monthly labor % bar chart
4. Balance Sheet - working capital (People+Rent+Utilities only), A/R boxes (TBD), fixed assets, Net to Owner
5. Valuation - 3 KPIs only (EBITDA, Private 5x, Corporate 10x), EBITDA buildUp, labor improvement table
6. Tax Planning - annual report reminder, per-owner: State PTE, Franchise/Excise, Personal Federal, Personal State by quarter
7. Best Practices - vs GPVA benchmarks, status: On Track / Review Recommended / Action Required
8. Bookkeeping Health - On Track / Review Recommended / Action Required only, never negative language
9. GPVA Internal (amber tab, no-print) - all action items + advisor notes + call agenda
...
PDF button: window.print()

TAB JAVASCRIPT — use exactly this:
<script>
function showTab(n){
  document.querySelectorAll('.tab-panel').forEach(function(p,i){p.style.display=i===n?'block':'none';});
  document.querySelectorAll('.tab-btn').forEach(function(t,i){t.className='tab-btn'+(i===n?' active':'');});
}
window.onload=function(){showTab(0);};
</script>

Each tab button: <button class="tab-btn" onclick="showTab(N)">Tab Name</button>
Each tab panel: <div class="tab-panel">...</div>
First panel visible by default, rest hidden via CSS: .tab-panel{display:none;}`
  }]
  ...
  for (const fc of fileContents) {
    ...
    if (fc.type === 'text') {
      ...
      content.push({ type: 'text', text: `\n--- FILE: ${fc.name} (${fc.ext}, ${Math.round(fc.content.length*0.75/1024)}KB) ---\n[Extract all financial data from this file]\n` })
    }
  }
...
async function saveToSupabase(fields, dashboardHtml) {
  ...
  try {
    ...
    if (existing?.length > 0) {
      ...
      await supabase.from('clients').update({ entity_type:f('entity'), state_practice:f('statePractice'), state_residence:f('stateResidence'), updated_at:new Date().toISOString() }).eq('id', clientId)
    } else {
    ...
    console.log('Saved to Supabase:', bizName)
